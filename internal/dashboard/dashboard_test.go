...
// ============================================================================
// Compact layout, component breakdown, inline SVG, and factor highlighting
// ============================================================================

func TestGenerateMulti_CompactLayoutWithTwoReports(t *testing.T) {
	reports := []ReportSummary{
		{
			Report: &confidence.Report{Title: "A", Score: intPtrT(90), Threshold: 75},
			Weight: 60, Path: "a.json",
		},
		{
			Report: &confidence.Report{Title: "B", Score: intPtrT(80), Threshold: 75},
			Weight: 40, Path: "b.json",
		},
	}
	aggregate := &confidence.Report{Title: "Agg", Score: intPtrT(86), Threshold: 75}
	...
	if !strings.Contains(html, "container-compact") {
		t.Error("2-report layout should use container-compact class")
	}
}

func TestGenerateMulti_NoCompactWithThreeReports(t *testing.T) {
	reports := []ReportSummary{
		{Report: &confidence.Report{Title: "A", Score: intPtrT(90), Threshold: 75}, Weight: 40, Path: "a.json"},
		{Report: &confidence.Report{Title: "B", Score: intPtrT(80), Threshold: 75}, Weight: 30, Path: "b.json"},
		{Report: &confidence.Report{Title: "C", Score: intPtrT(70), Threshold: 75}, Weight: 30, Path: "c.json"},
	}
	aggregate := &confidence.Report{Title: "Agg", Score: intPtrT(80), Threshold: 75}
	...
	// The class should not be applied to the container div (it exists in CSS definition)
	if strings.Contains(html, `class="container container-compact"`) {
		t.Error("3-report layout should NOT apply container-compact class to container div")
	}
}

func TestGenerateMulti_ComponentBreakdown(t *testing.T) {
	reports := []ReportSummary{
		{
			Report: &confidence.Report{Title: "API", Score: intPtrT(85), Threshold: 75},
			Weight: 60, Path: "api.json",
		},
		{
			Report: &confidence.Report{Title: "Web", Score: intPtrT(40), Threshold: 75},
			Weight: 40, Path: "web.json",
		},
	}
	aggregate := &confidence.Report{Title: "Agg", Score: intPtrT(67), Threshold: 75}
	...
	if !strings.Contains(html, "component-breakdown") {
		t.Error("output should contain component-breakdown section")
	}
	if !strings.Contains(html, "score: 85") {
		t.Error("component breakdown should show API score")
	}
	if !strings.Contains(html, "score: 40") {
		t.Error("component breakdown should show Web score")
	}
}

func TestGenerateMulti_InlineSVGNoXMLDeclaration(t *testing.T) {
	reports := []ReportSummary{
		{
			Report: &confidence.Report{Title: "A", Score: intPtrT(85), Threshold: 75},
			Weight: 100, Path: "a.json",
		},
	}
	aggregate := &confidence.Report{Title: "Agg", Score: intPtrT(85), Threshold: 75}
	...
	if strings.Contains(html, "<?xml") {
		t.Error("inline SVGs should not contain XML processing instruction")
	}
	if strings.Contains(html, "Generated by SVGo") {
		t.Error("inline SVGs should not contain SVGo comment")
	}
	if !strings.Contains(html, "viewBox=") {
		t.Error("inline SVGs should contain viewBox for proper scaling")
	}
}

func TestGenerateMulti_TransparentGaugeBG(t *testing.T) {
	reports := []ReportSummary{
		{
			Report: &confidence.Report{Title: "A", Score: intPtrT(85), Threshold: 75},
			Weight: 100, Path: "a.json",
	...
	if strings.Contains(html, `<rect x="0" y="0"`) {
		t.Error("embedded gauge SVGs should not contain background rect")
	}
}

func TestGenerateMulti_FactorFailedHighlight(t *testing.T) {
	reports := []ReportSummary{
		{
			Report: &confidence.Report{
				Title:     "Test",
				...
				Threshold: 75,
				Factors: []confidence.Factor{
					{Name: "Good Factor", Score: 95, Threshold: 80},
					{Name: "Bad Factor", Score: 30, Threshold: 80},
				},
			},
			Weight: 100, Path: "test.json",
		},
	}
	aggregate := &confidence.Report{Title: "Agg", Score: intPtrT(50), Threshold: 75}
	...
	if !strings.Contains(html, "factor-failed") {
		t.Error("factors below threshold should have factor-failed class")
	}
	if !strings.Contains(html, "report-failed") {
		t.Error("failing report card should have report-failed class")
	}
}

func TestGenerateMulti_FragmentCompactLayout(t *testing.T) {
	reports := []ReportSummary{
		{
			Report: &confidence.Report{Title: "A", Score: intPtrT(90), Threshold: 75},
			Weight: 60, Path: "a.json",
	...
	var buf bytes.Buffer
	err := GenerateMulti(&buf, reports, aggregate, MultiOptions{Fragment: true})
	if err != nil {
		t.Fatalf("GenerateMulti() error = %v", err)
	}
	html := buf.String()

	if !strings.Contains(html, "confvis-dashboard-compact") {
		t.Error("2-report fragment layout should use confvis-dashboard-compact class")
	}
	if !strings.Contains(html, "component-breakdown") {
		t.Error("fragment should contain component-breakdown section")
	}
}
